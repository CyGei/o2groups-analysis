...
predictors <- c(
  ...
  "INCUB_mean",
  "INCUB_sd",
  "WithinGroup Transmissions",
  "TotalGroup Transmissions",
  "N_pairs",
  "WithinGroup Beta"
)
...
outcomes_long <- outcomes_df %>%
  ...
  rename(
    ...
    "R0" = r0,
    "% of Intro" = intro_prop,
    "WithinGroup Transmissions" = successes,
    "TotalGroup Transmissions" = trials,
    "WithinGroup Beta" = beta
  ) %>%
...
f_ref <-
  cell_df %>% filter(outcome_name == "bias" &
                       predictor_name == "WithinGroup Beta") %>%
  .$frequency %>% max()
...
p_custom_hexbin <-
  ...
  scale_fill_gradientn(colours = c("#3679c0", "black", "#ff007f"),
                       values = scales::rescale(c(-0.5, -0.05, 0, 0.05, 0.5)),
...
ggsave(
  here("analysis/simulation/plots", "custom_hexbin4.png"),
