...
bias_formulas <- list(
  ...
  bias ~ size_freq,
  bias ~ size,
  bias ~ log(size_freq) * size_freq_cat15 + log(n_trans) * n_trans_cat,
...
#   slice(1) %>%
#   pull(Formula)
final_formula <- bias ~ log(size_freq) * size_freq_cat10 + log(n_cases) * n_cases_cat15
...
sensitivity_formulas <- list(
  ...
  car::logit(sensitivity2) ~ delta_type * abs_delta + size_freq_cat05,
  ...
  car::logit(sensitivity2) ~ delta_type * abs_delta + n_cases + size_freq
  )
#logit_trans()
#glm binomial
...
data %>%
  select(sensitivity2,
         ...
         abs_delta,
         delta) %>%
  pivot_longer(-c(sensitivity2, delta_type),
               names_to = "predictor") %>%
  drop_na() %>%
  ggplot(aes(x = value, y = car::logit(sensitivity2))) +
...
sensitivity_results <- compare_models(
  ...
  folds = folds,
  metrics = metrics
) #rank-deficient solely because we do not use delta_type = neutral for sensitivity
...
# take best rsq model
final_formula <- car::logit(sensitivity2) ~ delta_type * abs_delta + n_cases

# For supplementary material, showing the effect of Delta only
#final_formula <- car::logit(sensitivity2) ~ delta_type * abs_delta
...
last_fit %>%
  collect_predictions() %>%
  ggplot(aes(x = .pred, y = `car::logit(sensitivity2)`)) +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "Predicted", y = "Observed")
...
# take best rsq model
final_formula <-sensitivity ~ delta_type * abs_delta + n_cases

# For supplementary material, showing the effect of Delta only
#final_formula <-sensitivity ~ delta_type * abs_delta

last_fit <- workflow() %>%
  add_model(linear_reg() %>%
              set_engine("glm", family = stats::binomial(link = "logit"), weights = 100)) %>%
  add_formula(as.formula(final_formula)) %>%
  last_fit(split)
...
final_formula <- specificity ~ sd_peak_date
